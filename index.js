...
function checkData(data) {
  try {
    if (Object.keys(data).length) {
      return data;
    } else throw new Error("Об'єкт пустий");
  } catch (err) {
    return err.message;
  }
...
function getBookById(books, id) {
  ...
  if (!book) {
    return `TypeError:Книга з ID ${id} не знайдена!`;
  } else return `Книга: ${book.title}`;
...
function findEvenNumber(numbers) {
  ...
  if (evenNumber === undefined) {
    return `TypeError: У масиві немає чисел, що діляться на 2 без остачі!`;
...
function validateUser(user) {
  try {
    ...
    if (!user.name) {
      throw { message: "Ім'я користувача не вказано!", cause: user };
    }
    if (!user.email) {
      throw { message: "Email користувача не вказано!", cause: user };
    }
...
function processData(data) {
  try {
    ...
    return "Елемент з індексом ${index} має бути числом!";
  }
